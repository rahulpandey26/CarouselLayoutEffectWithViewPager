...
public class CarouselPagerAdapter extends FragmentPagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public Fragment getItem(int position) {
        ...
        try {
            if (position == HomePage.FIRST_PAGE)
                scale = BIG_SCALE;
            else
                scale = SMALL_SCALE;

            position = position % HomePage.count;

        } catch (Exception e) {
            e.printStackTrace();
        }
        return ItemFragment.newInstance(context, position, scale);
        //return new ItemFragment(context ,position , scale);
    }
    ...
    public int getCount() {
        int count = 0;
        try {
            count = HomePage.count * HomePage.LOOPS;
        } catch (Exception e) {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        // for showing bigger image in animation form which is current selected image
        try {
